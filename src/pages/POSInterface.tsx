...
const networkStatus = useNetworkStatus();
  ...
  // Focus automatique sur l'input badge après reset
  useEffect(() => {
  ...
  useEffect(() => {
    // Vérifier que articles est bien un tableau avant de filtrer
    if (Array.isArray(articles)) {
      ...
      // Si articles n'est pas un tableau, initialiser avec un tableau vide
      setFilteredArticles([]);
    }
  }, [articles, searchTerm]);

  // Update lastSync quand articles change
  useEffect(() => {
  ...
  const handleBadgeValidation = async () => {
    if (!badgeCode.trim()) {
      uiToast({
        title: "Code badge requis",
        description: "Veuillez saisir un code badge",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleTransactionValidation = async () => {
    ...
    if (result.success) {
      ...
      uiToast({
        ...
        description: `Ticket: ${result.data?.numeroTicket}`,
      });
      // Reset après un délai pour voir le message
      setTimeout(() => {
        setBadgeCode('');
        setSearchTerm('');
        badgeInputRef.current?.focus();
  ...
  const handleAddToCart = (article: ArticleDTO) => {
    addToCart(article);
    // Toast déjà géré dans le hook usePOS
  };
  ...
  // Impression ticket
  const printTicket = (transactionData: any) => {
    const printContent = `
      <!DOCTYPE html>
      <html>
        <head>
        ...
        <body>
          ...
          <table>
            ${transactionData.articles?.map((article: any) => `
              <tr>
                <td>${article.nom} x${article.quantite}</td>
                <td class="right">${article.montantTotal?.toFixed(2) || '0.00'} MAD</td>
              </tr>
              <tr>
                <td style="font-size: 10px; color: #666;">
                  Prix unit: ${article.prixUnitaire?.toFixed(2) || '0.00'} MAD | Subv: ${article.subventionTotale?.toFixed(2) || '0.00'} MAD
                </td>
          ...
          <table>
            ...
            <tr>
              <td>Nouveau solde:</td>
              <td class="right">${transactionData.nouveauSolde?.toFixed(2) || '0.00'} MAD</td>
            </tr>
          </table>
          <div class="separator"></div>
          <div class="center">
  ...
  useEffect(() => {
    const handleKeyPress = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case 'F3':
          e.preventDefault();
          if (cart.length > 0 && currentUser && !isValidationModalOpen) {
  ...
  }, [cart.length, currentUser, isValidationModalOpen]);
